...
import esa.mylibrary.utils.MyDexUtil
...
//                            setImageResource(R.drawable.baseline_photo_24)
                            ...
                            typeface = MyIcon.getTypeface(context, "fonts/iconfont.ttf")
...
//                            ShowToastMessage((o as File).absolutePath)
    ...
    fun toActivity(sharedElement: View, objClass: Class<*>, par: JSONObject) {
        val intent = Intent(binding.root.context, objClass)

        for (key in par.keys()) {
            intent.putExtra(key, par.getString(key))
        }
